...
const NewsEvents = () => {
  ...
  return (
    <>
      <div className="w-screen h-[600px] bg-slate-400 pt-10 pl-5 flex justify-around overflow-x-hidden">
        <div className="h-[505px] w-[646px] bg-white overflow-hidden">
          ...
          <div className="h-[411px] mt-5 transition-transform duration-500 overflow-hidden">
            <div
              style={{
                ...
                transform: `translateX(-${slide * (299 + 48)}px)`,
                width: `${totalDivs * (299 + 48) - 48}px`,
              }}
            >
              {Array.from({ length: totalDivs }).map((_, index) => (
                <div
                  key={index}
                  className={`h-[411px] w-[299px] border-b-[#463BAC] border-b-8 ${index > 0 ? 'ml-[48px]' : ''}`}
                  style={{
